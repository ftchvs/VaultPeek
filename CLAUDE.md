...
- `Routes/` — REST endpoints (Account, Link, Status, Transaction, Budget, Investment, Review, MerchantLogo, Webhook, Billing). Investment routes (AND-644) proxy Plaid Investments holdings + transactions; Review routes (AND-552) back the **opt-in** server-synced review state.
- `Plaid/` — `PlaidClient` + `PlaidModels`.
- `Storage/` — Fluent models/migrations (`Database.swift`), `TokenStore`, `PlaidTokenVault` (Keychain), plus per-feature stores (`BudgetStore`, `ReviewStateStore`, `WebhookEventStore`, `BillingSubscriptionStore`). The `CreateReviewState` migration (AND-552) persists synced review state only when the user opts in.
...
- `Services/` — `ServerClient` (HTTP), `NotificationService`, `LaunchService`, `LocalAIInsightsService`, on-device Foundation Models categorizers (`FoundationModelsMerchantCategorizer`/`FoundationModelsIncomeCategorizer`), `RecoveryActionDispatcher`, `GoalsStore` (local-first goals), `CategoryEditorModel` (budgeting-v2 category/group editor, AND-547), `ReviewSyncService` (opt-in review-state sync, AND-552), `HapticFeedback`.
