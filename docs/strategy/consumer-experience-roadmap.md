...
5. **Footer**: "Computed on this Mac. Nothing left your machine." + a single prominent "Go to dashboard" button (`.borderedProminent`, `.controlSize(.small)` like `DashboardOverviewFallbackBanner`). Dismissal sets the flag; the snapshot never auto-reappears.
