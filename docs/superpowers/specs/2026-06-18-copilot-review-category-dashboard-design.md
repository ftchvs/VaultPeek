...
- **New (PlaidBarCore):** `CategoryGroup` enum + `SpendingCategory.group`; `CategoryDashboardBuilder` (pure rollup, `asOf:`/`Calendar` injected, `Sendable`); extracted `EffectiveCategoryResolver` (lifted from `TransactionReviewInbox.resolveCategory:380-404`).
- **Changed (the fix):** `CategoryBudgetPlanner.netSpendByCategory` gains optional `metadata`/`rules` params (default nil → existing tests pass); skips excluded/transfer rows; resolves effective category. `AppState.categoryBudgetPresentation:1251` passes live metadata/rules.
...
- **Edge cases:** pending→posted (already handled, de-dupe in dashboard); transfers/credit-card payments (exclude via override); NL-suggested vs persisted category (display-only, stays `.uncategorized` until approved); over-at-leaf-but-under-at-group (independent status); multi-account aggregation vs account-filter; empty/first-run (suggestions only, no false "over"); refunds (signed-amount netting).
