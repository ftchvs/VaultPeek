...
struct PlaidBarTests {
    ...
    func windowFirstGoalsProgressUsesPrivacyMask() throws {
        ...
        let goalsCardStart = try #require(planningSource.range(of: "private var goalsCard: some View"))
        let goalsCardBody = String(planningSource[goalsCardStart.lowerBound...].prefix(1_000))
        let maskedVisibleGuard = try #require(goalsCardBody.range(of: "if isMasked"))
        let emptyVisibleGuard = try #require(goalsCardBody.range(of: "else if summary.isEmpty"))
        #expect(maskedVisibleGuard.lowerBound < emptyVisibleGuard.lowerBound)
