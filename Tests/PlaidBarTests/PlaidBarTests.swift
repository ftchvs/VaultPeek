...
struct PlaidBarTests {
    ...
    func windowFirstGoalsProgressUsesPrivacyMask() throws {
        ...
        let insightsSource = try String(
            contentsOf: root.appending(path: "Sources/PlaidBar/Views/Destinations/InsightsDestinationView.swift"),
            encoding: .utf8
        )
        ...
        #expect(planningSource.contains("Label(\"Goal details hidden\", systemImage: \"lock.fill\")"))
        #expect(planningSource.contains("} else if summary.isEmpty {"))
        #expect(planningSource.contains("VaultPeek is private. Open Goals after unlocking to review goal details."))
        ...
        let planningGoalsCardStart = try #require(planningSource.range(of: "private var goalsCard: some View"))
        let planningGoalsCardBody = String(planningSource[planningGoalsCardStart.lowerBound...].prefix(1_000))
        let planningVisibleMaskedGuard = try #require(planningGoalsCardBody.range(of: "if isMasked"))
        let planningVisibleEmptyGuard = try #require(planningGoalsCardBody.range(of: "else if summary.isEmpty"))
        #expect(planningVisibleMaskedGuard.lowerBound < planningVisibleEmptyGuard.lowerBound)
        let planningAccessibilityLabelStart = try #require(planningSource.range(of: "private func planningGoalsAccessibilityLabel"))
        let planningAccessibilityLabelBody = String(planningSource[planningAccessibilityLabelStart.lowerBound...].prefix(600))
        let planningMaskedGuard = try #require(planningAccessibilityLabelBody.range(of: "if isMasked"))
        let planningEmptyGuard = try #require(planningAccessibilityLabelBody.range(of: "guard !summary.isEmpty"))
        #expect(planningMaskedGuard.lowerBound < planningEmptyGuard.lowerBound)

        #expect(!insightsSource.contains(#"\(summary.overallPercent)% of total"#))
        #expect(insightsSource.contains("goalsPercent(summary.overallPercent)"))
        #expect(insightsSource.contains("if isMasked"))
        #expect(insightsSource.contains("ProgressView(value: summary.overallFraction)"))
        #expect(!insightsSource.contains("Label(summary.goalCountLabel"))
        #expect(insightsSource.contains("Label(goalsSecondaryLabel(summary)"))
        #expect(insightsSource.contains("isMasked ? \"Goal details hidden\" : summary.goalCountLabel"))
        #expect(insightsSource.contains("Label(\"Goal details hidden\", systemImage: \"lock.fill\")"))
        #expect(insightsSource.contains("} else if summary.isEmpty {"))
        #expect(insightsSource.contains("VaultPeek is private. Open Goals after unlocking to review goal details."))
        #expect(insightsSource.contains("if !isMasked, summary.fundedCount > 0"))
        #expect(insightsSource.contains("if !isMasked, summary.behindCount > 0"))
        #expect(insightsSource.contains("goalsAccessibilityLabel(summary)"))
        #expect(insightsSource.contains("if isMasked { return \"Goals: details hidden while VaultPeek is private.\" }"))
        let insightsGoalsCardStart = try #require(insightsSource.range(of: "private var goalsCard: some View"))
        let insightsGoalsCardBody = String(insightsSource[insightsGoalsCardStart.lowerBound...].prefix(1_000))
        let insightsVisibleMaskedGuard = try #require(insightsGoalsCardBody.range(of: "if isMasked"))
        let insightsVisibleEmptyGuard = try #require(insightsGoalsCardBody.range(of: "else if summary.isEmpty"))
        #expect(insightsVisibleMaskedGuard.lowerBound < insightsVisibleEmptyGuard.lowerBound)
        let insightsAccessibilityLabelStart = try #require(insightsSource.range(of: "private func goalsAccessibilityLabel"))
        let insightsAccessibilityLabelBody = String(insightsSource[insightsAccessibilityLabelStart.lowerBound...].prefix(600))
        let insightsMaskedGuard = try #require(insightsAccessibilityLabelBody.range(of: "if isMasked"))
        let insightsEmptyGuard = try #require(insightsAccessibilityLabelBody.range(of: "guard !summary.isEmpty"))
        #expect(insightsMaskedGuard.lowerBound < insightsEmptyGuard.lowerBound)
    }
